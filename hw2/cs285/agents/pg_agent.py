...
class PGAgent(nn.Module):
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        involve t)!
        """
        res = np.asarray(rewards, dtype=float)
        discounts = np.power(self.gamma, np.arange(len(res)))
        total_return = np.sum(discounts * res)
        # Return the same value for each timestep in the trajectory
        return np.full_like(res, total_return, dtype=float)
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        # discounts = np.power(self.gamma, np.arange(rtg.size))
        # discounted = rtg * discounts
        # # Reverse cumsum of discounted rewards, then remove the leading discount factor
        # rtg = np.cumsum(discounted[::-1])[::-1] / discounts
